
<template>
  <div class="panel">
    <div class="box-buttons" >
        <vxv_button round class="item edit"
          @click="edit = !edit"
          >
          <vxv_icon
            :color="edit ? '#ffc107' : 'white'"
            :name="edit ? 'content-save-all-outline' : 'pencil'"
            />
        </vxv_button>
      <div class="lineV_out"><div class="lineV_in"></div></div>

        <vxv_button class="item del" v-if="edit && btnDel"
           round @click="$emit('delRow')"
           >
          <vxv_icon color="white" name="delete" />
        </vxv_button>
        <!-- <div class="tip" >Удалить выделенную строку</div> -->
        <vxv_button class="item plus" v-if="edit && btnPlus"
           round @click="$emit('plusRow')"
           >
          <vxv_icon color="white" name="plus-circle" />
        </vxv_button>

      <!-- <img class="icon" src="./vxv_icon/pencil-circle.svg" alt="dots icon"> -->

    </div>

    <div class="title">{{ title }}</div>

    <vxv_input_search v-if="rows"
      v-model="rows"
      :nameTable="nameTable"
    ></vxv_input_search>

  </div>
</template>


<script setup>
import { toRefs, watch } from 'vue'
import vxv_icon from "./vxv_icon.vue"
import vxv_button from "./vxv_button.vue"
import vxv_input_search from "./vxv_input_search.vue"

const props = defineProps({
  title: {type: String, default: "title"},
  btnDel: {type: Boolean, default: false},
  btnPlus: {type: Boolean, default: false},
  nameTable: {type: String, default: null},
})
const {
  title,
  btnDel,
  btnPlus,
  nameTable,
} = toRefs(props)

const emit = defineEmits(['editFalse', 'delRow', 'plusRow'])
const edit = defineModel("edit", {type: Boolean, default: false})
// const rows = defineModel("rows", {type: Array, default: null})
const rows = defineModel("rows")


watch(() => edit.value, (v) => {
  if (v === false) {
    emit('editFalse')
  }
})
</script>


<style lang="sass" scoped>
@import "src/components/vxv_styles/base.sass"
.panel
  background-color: $blue-grey
  margin-bottom: 2.5px
  color: white
  display: flex
  align-items: center
  text-align: center
  position: sticky
  top: 0
  left: 0
  right: 0
  bottom: 0
.box-buttons
  display: flex
  height: inherit
  min-width: 35px
  padding: 0 2.5px
.item
  height: 40px
  position: relative
  min-width: 40px
  padding: 2.5px
  margin: 2.5px
  &:hover
    background-color: $blue-grey-4

// .edit
// .del
  // z-index: 1
  // &:after
  //   // &:hover:after
  //   position: absolute
  //   z-index: 50
  //   top: 415px
  //   top: -12px
  //   left: 750px
  //   color: red
  //   background-color: $green
  //   content: 'Удалить выделенную строку'
  //   // display: inline-block

.title
  text-horizontal: center
  height: inherit
  display: flex
  justify-content: center
  align-items: center
  margin: auto

.lineV_out
  height: inherit
  display: flex
  justify-content: center
  align-items: center
  // padding: 0 2.5px
  margin: 0 2.5px
.lineV_in
  width: 1.5px
  background-color: white
  height: 80%


</style>
